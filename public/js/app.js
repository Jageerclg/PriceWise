...
const PriceWiseApp = (function() {
    ...
    function bindCopyProductLink(product) {
        const button = Utils.selectElement('#copy-product-link');
        const status = Utils.selectElement('#copy-product-status');
        if (!button || !product || button.dataset.bound === 'true') return;
        button.dataset.bound = 'true';
        button.addEventListener('click', async () => {
            try {
                await ProductShareURL.copy(product);
                if (status) status.textContent = 'Product link copied.';
            } catch (error) {
                if (status) status.textContent = error.message || 'Unable to copy the product link.';
            }
        });
    }
