...
const ProductsModule = (function() {
    ...
    /**
     * Format a fetchedAt timestamp for the live-data source label.
     * Never claims real-time; shows the fetch date or 'recently'.
     * @param {string} fetchedAt - ISO timestamp
     * @returns {string} Human-readable label
     */
    function formatLiveFallback(fetchedAt) {
    ...
    function renderProductDetails(product, container) {
        ...
        if (productSource) {
            Utils.setTextContent(productSource, `Source: ${product.sourceType === 'mock' ? 'Mock provider data' : product.sourceType === 'live' ? `Live data · fetched ${formatLiveFallback(product.fetchedAt)}` : 'Local demo catalog'}`);
        }
        ...
        // Price intelligence (discount, history, store comparison) activates
        // only for live imported products; catalog products hide those sections.
        if (typeof PriceIntelligence !== 'undefined' && PriceIntelligence.init) {
            PriceIntelligence.init(product);
        }
    }
