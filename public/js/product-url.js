...
const ProductURLModule = (function() {
    ...
    function isSupportedHost(hostname, apexDomain) {
        // Accept the apex domain and any subdomain (www., m., smile., …)
        // while rejecting lookalikes: 'fakeamazon.in' and
        // 'amazon.in.evil.com' do not equal or end with '.amazon.in'.
        const host = String(hostname || '').toLowerCase();
        const apex = String(apexDomain || '').toLowerCase();
    ...
    function detectStoreFromUrl(urlValue) {
        ...
        try {
            ...
            for (const storeKey of Object.keys(SUPPORTED_STORES)) {
                ...
                if (store.domains.some((apex) => isSupportedHost(hostname, apex))) {
                    return {
                        key: storeKey,
                        name: store.name,
                        hostname,
        ...
        try {
            ...
            if (storeKey === 'flipkart') {
                const match = normalizedUrl.match(/\/p\/(?:itm[A-Za-z0-9]+|[A-Za-z0-9]+)/i) ||
                    ...
                    normalizedUrl.match(/\/itm([A-Za-z0-9]+)/i);
                if (match) {
                    return match[1] || match[0].split('/').pop();
                }
                return null;
    ...
    function initializeSearchForms() {
        ...
        forms.forEach(form => {
            ...
            Utils.addEventListener(form, 'submit', async (event) => {
                ...
                if (typeof ProductShareURL !== 'undefined') {
                    if (!ProductsModule.isLoaded()) {
                        try { await ProductsModule.loadProducts(); } catch (_) { /* normal navigation will show the load error */ }
                    }
                    const localLink = ProductShareURL.parse(searchTerm, ProductsModule.getProducts());
                    if (localLink.matched) {
                        event.preventDefault();
                        if (!localLink.valid) {
                            setStatusMessage(localLink.error, 'error');
                            return;
                        }
                        window.location.assign(`product-details.html?id=${encodeURIComponent(localLink.product.id)}`);
                        return;
                    }
                }
                ...
                if (validation.isValid && validation.supported) {
                    ...
                    // Open the placeholder tab synchronously in the submit
                    // gesture (popup-safe); it is navigated after the async
                    // import completes, or closed if the import fails.
                    const detailsTab = openBlankTab();
                    const importResult = await importProductFromUrl(searchTerm);
                    showImportedProduct(importResult, detailsTab);
                    return;
                }
    ...
    function openBlankTab() {
        // Must be called synchronously inside the user gesture (submit)
        // so browser popup blockers allow the new tab. Returns the tab
        // handle, or null when popups are blocked/unavailable.
        try {
            if (typeof window !== 'undefined' && typeof window.open === 'function') {
                return window.open('about:blank', '_blank');
            }
        } catch (error) {
            console.warn('ProductURLModule: new tab blocked', error);
        }
        return null;
    }
    ...
    function closeTab(tab) {
        try {
            ...
            console.warn('ProductURLModule: tab close failed', error);
        }
    }

    function navigateTab(tab, targetUrl) {
        // Returns true when the new tab was used, false for same-tab fallback.
        if (tab && !tab.closed) {
            try {
                tab.location.href = targetUrl;
                try {
                    tab.opener = null;
                } catch (_) { /* opener unlink is best-effort */ }
                return true;
            } catch (error) {
                console.warn('ProductURLModule: new-tab navigation failed', error);
            }
        }
        try {
    ...
    function showImportedProduct(importResult, detailsTab) {
        // On failure: never open/navigate anywhere; close the placeholder
        // tab (if any) and report the friendly error on the current page.
        if (!importResult || !importResult.ok || !importResult.product) {
            closeTab(detailsTab);
            // Backend errors arrive as {code, message} objects; display
            // the friendly message text instead of "[object Object]".
            const importError = importResult && importResult.error;
            const errorText = typeof importError === 'string'
                ? importError
                : (importError && importError.message) || 'We couldn\'t retrieve product information from this URL.';
            setStatusMessage(errorText, 'error');
            return { ok: false, openedNewTab: false };
        }
        ...
        setStatusMessage('Product loaded. Opening in a new tab...', 'success');
        const openedNewTab = navigateTab(detailsTab, targetUrl);
        return { ok: true, openedNewTab, targetUrl };
    }
