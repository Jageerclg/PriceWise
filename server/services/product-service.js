...
class ProductService {
  constructor({ providerMode = 'mock', snapshotService = null } = {}) {
    this.providerMode = providerMode;
    this.providers = {
      amazon: new AmazonProvider({ mode: providerMode }),
      flipkart: new FlipkartProvider({ mode: providerMode })
    };
    // Snapshot tracking for live imports. Lazily created so existing
    // constructions (tests, offline mode) stay dependency-free until used.
  ...
  normalizeProduct(product) {
    ...
    return {
      ...
      ...(typeof product.mrp === 'number' && Number.isFinite(product.mrp) && product.mrp > 0
        ? { mrp: product.mrp }
        : {}),
      ...(typeof product.discountPercent === 'number' && Number.isFinite(product.discountPercent) && product.discountPercent >= 0
  ...
  async importProductFromUrl(url) {
    ...
    try {
      ...
      const normalized = this.normalizeProduct(result.product);

      // Persist a historical snapshot for REAL live imports only.
      // Best-effort: a snapshot failure must never break the import itself.
      if (normalized && result.product.sourceType === 'live') {
        try {
          await this.getSnapshotService().recordSnapshot(normalized);
        } catch (snapshotError) {
          console.error(
            'ProductService snapshot failed:',
            snapshotError && snapshotError.message ? snapshotError.message : snapshotError
          );
        }
      }
